...
static VALUE c_parse(VALUE self, VALUE input, VALUE f_strip, VALUE f_inline, VALUE f_disable_mentions, VALUE f_allow_color, VALUE f_max_thumbs) {
  if (NIL_P(input)) {
    return Qnil;
  }

  StringValue(input);
  StateMachine* sm = init_machine(RSTRING_PTR(input), RSTRING_LEN(input), RTEST(f_strip), RTEST(f_inline), !RTEST(f_disable_mentions), RTEST(f_allow_color), FIX2LONG(f_max_thumbs));
  if (!parse_helper(sm)) {
...
void Init_dtext() {
  ...
  rb_define_singleton_method(mDTextRagel, "c_parse", c_parse, 6);
}
